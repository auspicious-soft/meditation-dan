...
const Page = () => {
  ...
  const handleApprove = async (requestId: string) => {
    try {
      const response = await updateCompanyJoinRequest(`/admin/company-join-requests/${requestId}?status=approve`
      );
      if (response.data.success) {
  ...
  const handleDeny = async () => {
    ...
    try {
      const response = await updateCompanyJoinRequest(`/admin/company-join-requests/${selectedRequestId}?status=deny`
      );
      if (response.data.success) {
            ...
            <TableBody>
              {currentRequests.map((request) => (
                <TableRow key={request._id} className="border-0 text-sm font-normal hover:bg-transparent">
                  <TableCell className="py-4 w-1/4">{request.companyId?.identifier}</TableCell>
                  ...
                  <TableCell className="text-right py-4 w-1/4 whitespace-nowrap min-w-[120px]">
                    <div className="flex gap-x-2 justify-end">
                      <Button
                        className="bg-[#14AB00] hover:bg-[#14AB00]"
                        onClick={() => handleApprove(request.companyId?._id ?? "")}
                      ...
                      <Button
                        onClick={() => {
                          setSelectedRequestId(request.companyId?._id ?? "");
